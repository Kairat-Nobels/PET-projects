...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `ОЧКИ: ${state.game.score}`;
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	for (let i = frags.length - 1; i >= 0; i--) {
		...
		frag.project();
		if (
			frag.projected.y > centerY + targetHitRadius || frag.projected.x < fragLeftBound || frag.projected.x > fragRightBound || frag.z > cameraFadeEndZ
		) {
			frags.splice(i, 1);
			returnFrag(frag);
			continue;
